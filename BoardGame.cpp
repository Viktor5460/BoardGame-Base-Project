...
std::map<std::string, int> BoardGame::getRatings() const {
    return ratings;
}

std::map<std::string, std::string> BoardGame::getFeatures() const {
    return features;
}
...
bool BoardGame::addRating(const std::string& playerId, int rating) {
    ...
    // Проверяем, что игрок еще не оценивал эту игру
...
bool BoardGame::updateFeature(const std::string& featureName, const std::string& featureValue) {
    // Проверяем, что признак уже существует
    if (features.find(featureName) == features.end()) {
        return false;  // Признак не существует
    }
...
// Операторы сравнения по средней оценке
